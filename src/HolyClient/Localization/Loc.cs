...
public class Loc : ReactiveObject
{
    ...
    public string CurrentLanguage
    {
        ...
        set
        {
            if (value != null)
            {
                if (!AvailableLanguages.Contains(value)) AvailableLanguages.Add(value);

                if (!currentLanguage.Equals(value))
                {
                    var changingArgs = new CurrentLanguageChangingEventArgs(currentLanguage, value);
                    var changedArgs = new CurrentLanguageChangedEventArgs(currentLanguage, value);
    ...
    /// <summary>
    ///     The language to use if a translation is missing in the <see cref="CurrentLanguage" />.
    ///     <para />
    ///     If the translation is also missing in the <see cref="FallbackLanguage" /> the defaultText if specified is then
    ///     used.
    ///     <para />
    ...
    /// <summary>
    ///     if set to <c>true</c> use <see cref="FallbackLanguage" /> when a translation is missing.
    ///     <para />
    ///     if set to <c>false</c> <see cref="FallbackLanguage" /> is not used and the defaultText if specified is directly
    ///     used.
    ///     <para />
    ...
    /// </summary>
    public List<ITranslator> Translators { get; } = new();

    public List<IFormatter> Formatters { get; set; } = new()
    {
    ...
    /// </summary>
    public SortedDictionary<string, SortedDictionary<string, LocTranslation>> TranslationsDictionary { get; set; } =
        new();
    ...
    public static Loc Instance
    {
        get
        {
            if (instance == null)
                lock (lockObject)
                {
                    if (instance == null) instance = new Loc();
                }
    ...
    public SortedDictionary<string, SortedDictionary<string, string>> MissingTranslations { get; } = new();
    ...
    /// <summary>
    ///     To fire the event <see cref="PropertyChanged" /> for the specified property.
    ///     <para />
    ...
    ///     The language id in which to get the translation. To Specify if not
    ///     <see cref="CurrentLanguage" />
    /// </param>
    /// <returns>The translated text</returns>
    public virtual string Translate(string textId, string defaultText = null, string languageId = null)
    {
        if (string.IsNullOrEmpty(defaultText)) defaultText = textId;

        if (string.IsNullOrEmpty(languageId)) languageId = CurrentLanguage;
        ...
        return Translators
               ...
               (UseFallbackLanguage
                   ? Translators
                       ...
                       .Translate(textId ?? string.Empty, FallbackLanguage)
                   : null)
    ...
    protected virtual void CheckMissingTranslation(string textId, string defaultText)
    {
        if (LogOutMissingTranslations)
        {
            ...
            if (needLogUpdate)
                MissingTranslationFound?.Invoke(this,
                    new LocalizationMissingTranslationEventArgs(this, MissingTranslations, textId));
        }
    }
